<x-layouts.app>
    <x-slot:title>Edit Listing: {{ $job->title }} — TalentFlow</x-slot:title>

    <div class="max-w-2xl mx-auto space-y-6">
        <!-- Back Navigation -->
        <div>
            <a href="/jobs/{{ $job->id }}" class="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white transition duration-150">
        ...
        <div class="glass-card rounded-3xl p-8 sm:p-10 shadow-2xl relative overflow-hidden">
            ...
            <div class="mb-8">
                ...
                <p class="mt-2 text-slate-400 text-sm">
                    Make modifications to your posting and save to publish the updates instantly.
                </p>
            </div>
            ...
            <form method="POST" action="/jobs/{{ $job->id }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="salary" id="salary" value="{{ old('salary', $job->salary) }}" required
                           class="w-full bg-slate-900/50 border border-slate-800 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200"
                ...
                <!-- Submit Buttons -->
                <div class="pt-4 flex items-center justify-end gap-4 border-t border-slate-900 mt-6">
                    <a href="/jobs/{{ $job->id }}" class="px-5 py-3 text-sm font-semibold text-slate-400 hover:text-white transition duration-150">
                        Cancel
                    </a>
